...
import { ImagePlus, KeyRound, Lock, Save, Trash2, X } from "lucide-react";
import {
  buildTeamSections,
  getPlayerCategory,
  teamCategories,
...
export function PlayerManager({ initialItems }: { initialItems: Player[] }) {
  ...
  const handleUnlock = (event: FormEvent<HTMLFormElement>) => {
    ...
    if (password === ACCESS_KEY) {
      ...
      setUnlocked(true);
      setPassword("");
      setMessage("Panel desbloqueado. Ya puedes añadir y editar jugadores.");
      return;
    }
        ...
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            ...
            <p className="mt-2 max-w-lg text-sm leading-relaxed text-muted">
              Usa la misma clave de entrenamientos para añadir, editar o borrar
              jugadores.
            </p>
          </div>
        </div>
